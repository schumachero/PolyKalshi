...
def analyze_liquidity_and_efficiency():
    ...
    for _, row in df.iterrows():
        ...
        strategies = [
            {
                ...
                "k_opp_vol": row.get("k_yes_bid_vol"),
        ...
        for s in strategies:
            ...
            if not is_good_liquidity:
                analysis["fail_reason"].append(f"Low Liq (${min_executable_usd:.1f})")
            if not is_efficient:
                analysis["fail_reason"].append(f"Low Daily ROI ({daily_roi:.3f}%)")
            if not is_short_enough:
                analysis["fail_reason"].append(f"Too long ({days_to_res} days)")
    ...
    if not passed.empty:
        ...
        print(
            passed[
                ["kalshi_market", "strategy", "profit_pct", "daily_roi", "liquidity_usd"]
        ...
        print(
            analysis_df[
                ["kalshi_market", "strategy", "profit_pct", "daily_roi", "liquidity_usd", "fail_reason"]
            ].head(10).to_string(index=False)
        )
