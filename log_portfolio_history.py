...
def log_to_history(total_value):
    ...
    # Use UTC + 2 to match user's local time (consistant with dashboard)
    # The user is in GMT+2 (e.g. CET/CEST)
    now = (datetime.datetime.now() + datetime.timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")
